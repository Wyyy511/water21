...
def test_no_proxy_constraint_knows_uploaded_state():
    with open("WaterPulse_Demo_Upload_甘蔗_2025.xlsx","rb") as f:
        r=c.post("/api/agent/message",data={"message":"请分析，但不得使用任何代理值或假设。"},files={"file":("WaterPulse_Demo_Upload_甘蔗_2025.xlsx",f,"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}).json()
    assert r["status"]=="insufficient"; assert "已经读取" in r["message"] and "6 条采购记录" in r["message"]
