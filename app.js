...
function addAI(data){
  ...
  let provider='WaterPulse 本地说明';
  if(data.ai?.provider==='DeepSeek') provider=`DeepSeek · ${esc(data.ai.model||'')}`;
  else if(data.ai?.configured && data.ai?.reason==='http_error') provider=`DeepSeek 接口异常${data.ai.http_status?' · HTTP '+esc(data.ai.http_status):''} · 已自动兜底`;
  else if(data.ai?.configured && data.ai?.reason==='connection_error') provider='DeepSeek 连接异常 · 已自动兜底';
  else if(data.ai?.configured && data.ai?.retryable) provider='DeepSeek 本轮未返回完整结果 · 已自动兜底';
...
async function callAgent({message='',action='',scenario_type='',material='',year='',path='',failure_fraction='',inventory='',file=null}={}){
  ...
  }catch(e){removeTyping();addAI({message:'这次服务请求没有完成，但不是因为你缺少数据。请稍后重试；如果持续出现，可以查看右上角 AI 状态确认 DeepSeek 是否连通。',trace:[{step:'服务请求异常',status:'error',detail:String(e)}],ai:{provider:'local_fallback',configured:false}})}
...
function bindActions(root=document){
  root.querySelectorAll('.action-btn').forEach(btn=>btn.onclick=async()=>{
    ...
    if(a.action==='download_run_record'){if(!state.sessionId){toast('当前没有分析会话');return}location.href='/api/agent/run-record/'+encodeURIComponent(state.sessionId);return}
    addUser(a.label);
