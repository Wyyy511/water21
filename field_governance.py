...
def evaluate_user_records(records: list[dict] | None) -> dict[str, Any]:
    records=records or []; missing=[]; warnings=[]
    if not records:
        return {'status':'insufficient','missing_fields':['material','supplier_node','W'],
                'missing_details':['未提供采购/供应链记录'],'warnings':[],'unknown_share_by_material':{},
                'source':'field_dictionary.csv + field_gap_register.csv'}
    known_sums={}; explicit_unknown={}
    unknown_names={'unknown','未知','来源未知','未知来源','未识别','unmapped'}
    for idx,row in enumerate(records,start=1):
        mat=str(_val(row,'material','material_id') or '').strip()
        node=str(_val(row,'node_id','supplier_node_id','node_name') or '').strip()
        is_unknown=bool(row.get('unknown_flag')) or node.lower() in unknown_names
        w=_val(row,'purchase_weight','W')
        if not mat: missing.append({'row':idx,'field':'material','message':'缺少原材料'})
        if not node and not is_unknown:
            # Fully custom parameter rows may use a user node label later, but still need some node identity.
            missing.append({'row':idx,'field':'supplier_node','message':'缺少供应节点/地区'})
        try:
            wf=float(w)
            if wf<0 or wf>1: missing.append({'row':idx,'field':'W','message':'采购份额必须在 0–1'})
            if mat:
                if is_unknown: explicit_unknown[mat]=explicit_unknown.get(mat,0)+wf
                else: known_sums[mat]=known_sums.get(mat,0)+wf
        except Exception:
            missing.append({'row':idx,'field':'W','message':'缺少或无法识别采购份额'})
    unknown={}
    for mat in set(known_sums)|set(explicit_unknown):
        k=known_sums.get(mat,0); u=explicit_unknown.get(mat,0); total=k+u
        if total>1.0001:
            missing.append({'row':None,'field':'W','message':f'{mat} 采购份额合计 {total:.4f}>1'})
        else:
            unknown[mat]=round(u+max(0,1-total),6)
            if unknown[mat]>0:
                warnings.append(f'{mat} 已知可定位采购份额 {k:.4f}，Unknown={unknown[mat]:.4f}；系统不会自动归一化。')
    status='insufficient' if missing else ('partial' if warnings else 'success')
    return {'status':status,'missing_fields':sorted(set(x['field'] for x in missing)),
            'missing_details':[x['message'] if x['row'] is None else f"第{x['row']}行：{x['message']}" for x in missing],
            'warnings':warnings,'unknown_share_by_material':unknown,
            'source':'field_dictionary.csv + field_gap_register.csv'}
